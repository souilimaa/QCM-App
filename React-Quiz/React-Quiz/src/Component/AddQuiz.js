...
const AddQuiz = () => {
    ...
    const confirmNewMatiere = async () => {
        try {
            const response = await fetch('http://localhost:8080/Qcm/matieres/create', {
                method: 'POST',
                headers: {
    ...
    const onChange = (e) => {
        ...
        if (name === 'matiere' && value === 'nouvelleMatiere') {
    ...
    const onSubmit = async (e) => {
        ...
        const { professeurId, titre, matiere, description, duree, nombreQst, choixMultiple } = formData;
        const { questions } = formData;

        if (
            formData.titre.trim() === '' ||
            formData.matiere.trim() === '' ||
        ...
        try {
            console.log({ professeurId, titre, matiere, description, duree, nombreQst, choixMultiple });

            const qcmResponse = await fetch('http://localhost:8080/Qcm/qcms/create', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ professeurId, titre, matiere, description, duree, nombreQst, choixMultiple }),
            });

            if (!qcmResponse.ok) {
                const errorDetails = await qcmResponse.json(); 
                console.error('Failed to create QCM:', errorDetails.message);
                alert(`Failed to create QCM: ${errorDetails.message}`);
                return; 
            }
    ...
    const addQuestionToDB = async (questionData, idQcm) => {
        try {
            const response = await fetch('http://localhost:8080/Qcm/questions/add', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({  idQcm: idQcm, ennonce: questionData.ennonce }), 
            });
    ...
    const addChoiceToDB = async (choiceText, questionId, isCorrect) => {
        try {
            const response = await fetch('http://localhost:8080/Qcm/choix/add', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
