...
const QcmDetails = () => {
  ...
  useEffect(() => {
    const fetchQcmDetails = async () => {
      try {
        const response = await fetch(`http://localhost:8080//Qcm/qcms/questions/${qcmId}`);
        if (!response.ok) {
    ...
    const fetchQuestions = async () => {
      try {
        const response = await fetch(`http://localhost:8080//Qcm/qcms/questions/getQuestionsByQcmId/${qcmId}`);
        if (!response.ok) {
    ...
    const fetchChoicesForQuestion = async (questionId) => {
      try {
        const response = await fetch(`http://localhost:8080//Qcm/qcms/choices/choices/${questionId}`);
        if (!response.ok) {
  ...
  const deleteQuiz = async () => {
    try {
      const response = await fetch(`http://localhost:8080//Qcm/qcms/${qcmId}`, { method: 'DELETE' });
      if (!response.ok) {
